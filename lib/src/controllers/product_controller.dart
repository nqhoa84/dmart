...
//  List<Favorite> favorites = <Favorite>[];
  ...
  void listenForBestSaleProducts({bool nextPage = false}) async {
    ...
    if (this.bestSaleProducts == null) bestSaleProducts = [];
    pros.forEach((p) {
      setState(() {
        if (p.isValid) bestSaleProducts!.add(p);
      });
    });
  }
  ...
  Future<bool> listenForNewArrivals({bool nextPage = false}) async {
    ...
    if (this.newArrivalProducts == null) newArrivalProducts = [];
    setState(() {
      ps.forEach((element) {
        newArrivalProducts!.add(element);
      });
    });
    return true;
...
//    return true;
  ...
  void listenForSpecial4U({bool nextPage = false}) async {
    ...
    if (this.special4UProducts == null) special4UProducts = [];
    setState(() {
      ps.forEach((element) {
        special4UProducts!.add(element);
      });
    });
...
//    _favPageIdx = nextPage == false ? 1 : _favPageIdx + 1;
    ...
    stream.listen((Favorite? _favorite) {
      if (_favorite!.isValid) {
        setState(() {
          DmState.favorites.add(_favorite);
          print(
              'listenForFavorites favorites {id: ${_favorite.id} product ${_favorite.product!.toStringIdName()}');
        });
      }
    }, onError: (a) {
...
//      final List<Product> stream = await getRelatedProducts2(productId);
      ...
      if (this.relatedProducts == null) relatedProducts = [];
      setState(() {
...
//      }, onDone: () {});
  ...
  void listenForProduct(
      {int? productId, String? message, Function()? onComplete}) async {
  ...
  void listenForPromoProducts(int promoId, {bool nextPage = false}) async {
    ...
    final List<Product> ps =
        await getProductsByPromotion2(promoId, _promoPageIdx);
    if (this.promotionProducts == null) promotionProducts = [];
    setState(() {
      ps.forEach((element) {
        promotionProducts!.add(element);
      });
    });
  }
  ...
  Future<bool> listenForProductsByCategory(
      {int? id, String? message, bool nextPage = false}) async {
    _cateProductsPageIdx = nextPage == false ? 1 : _cateProductsPageIdx + 1;
    final List<Product> ps =
        await getProductsByCategory2(id!, _cateProductsPageIdx);
    if (this.categoriesProducts == null) categoriesProducts = [];
    setState(() {
      ps.forEach((element) {
        categoriesProducts!.add(element);
      });
    });
    return true;
//    if(id > 0) {
...
//    }
  ...
  void listenForCarts() async {
    try {
      ...
      stream.listen((Cart? _cart) {
        if (_cart!.isValid) carts!.add(_cart);
      }).onDone(() {
        setState(() {
  ...
  void addToCart(int productId,
      {int quality = 1, Function(bool)? onDone}) async {
    try {
