...
//  Address defaultAddress;
  ...
  ProfileInfoController({
    this.isPersonalChange,
    this.isAddrChanged,
    this.personalDetailFormKey,
    this.addressFormKey,
    this.changePassFormKey,
    this.newPass,
    this.currentPass,
    this.defaultAddress,
  }) {
...
//    return u.isValid;
  ...
  Future<bool> changePwd() async {
    ...
    try {
      setState(() => loading = true);
      re = await userRepo.changePwd(
          user!.phoneWith855, this.currentPass!, this.newPass!);
      if (re == true) {
    ...
    stream.listen(
      ...
      onDone: () {
        if (_allAddrs!.length > 0 && this.defaultAddress == null) {
          setState(() {
            this.defaultAddress = _allAddrs![0];
          });
        }
      },
    );
