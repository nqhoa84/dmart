...
Future<http.Response> post({required String shortUrl, Map? body}) async {
  var url =
      Uri.parse('${GlobalConfiguration().getValue('api_base_url')}$shortUrl');
  print('url: $url \nParas: $body');
  return await http.Client().post(
    url,
    headers: {
      HttpHeaders.contentTypeHeader: 'application/json',
      'language': DmState.isKhmer ? 'kh' : 'en'
    },
...
Future<http.Response> get(
    {required String shortUrl, Map<String, String>? paras}) async {
  var url =
      Uri.parse('${GlobalConfiguration().getValue('api_base_url')}$shortUrl');
  print('url: $url \nParas: $paras');
  if (paras != null) {
    Map<String, String>? newParas = {
      HttpHeaders.contentTypeHeader: 'application/json',
      'language': DmState.isKhmer ? 'kh' : 'en'
    };
