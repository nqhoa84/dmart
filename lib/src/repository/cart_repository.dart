...
//  '${GlobalConfiguration().getString('api_base_url')}carts?${_apiToken}with=product;product.store;options&search=user_id:${_user.id}&searchFields=user_id:=';
  ...
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}carts?with=product;product.store;options&searchFields=user_id:=';
  print('getCarts $url');
...
//  cart.userId = _user.id;
  var url = Uri.parse('${GlobalConfiguration().getValue('api_base_url')}carts');
  print('addCart $url');
...
//  final client = new http.Client();
  final response = await http.Client().post(
    ...
    body: json.encode({"product_id": '$productId', "quantity": '$quality'}),
  );
  List<Cart> re = [];
