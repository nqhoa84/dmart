        ...
        child: CustomScrollView(
          slivers: <Widget>[
            ...
            createSilverTopMenu(context,
                haveBackIcon: true, title: S.current.deliveryAddresses),
            SliverList(
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                    child:
                        Text('${a.isDefault! ? S.current.yes : S.current.no}')),
              ],
            ),
          ),
          Divider(thickness: 1, height: 2),
...
//                    shape: StadiumBorder(),
  ...
  void onPressEditAddress(Address a) {
    ...
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (_) => AddressScreen(address: a)))
        .then((value) {
      print('-------value return from pop $value');
      if (value != null && value is List<Address>) {
        setState(() {
  ...
  void onPressDeleteAddress(Address a) {
    ...
    showAlertDialog(context, onPressedOK: _ok, onPressedCancel: _cancel);
  }
