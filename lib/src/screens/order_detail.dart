...
class OrderDetailScreen extends StatefulWidget {
  Order? odr;
  int orderId;
  OrderDetailScreen({Order? order, this.orderId = -1}) {
    this.odr = order;
  }
  ...
  _OrderDetailScreenState createState() =>
      _OrderDetailScreenState(this.odr!, this.orderId);
}
...
class _OrderDetailScreenState extends StateMVC<OrderDetailScreen> {
  ...
  _OrderDetailScreenState(this.odr, this.orderId)
      : super(OrderController(context: null)) {
          ...
          child: CustomScrollView(
            slivers: <Widget>[
              ...
              createSilverTopMenu(context,
                  haveBackIcon: true, title: S.current.myOrder),
              SliverList(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(
              S.current.orderValue, getDisplayMoney(_con.order!.orderVal),
              isBold: true),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(
              S.current.serviceFee, getDisplayMoney(_con.order!.serviceFee)),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(
              S.current.deliveryFee, getDisplayMoney(_con.order!.deliveryFee)),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(S.current.discountVoucher,
              getDisplayMoney(_con.order!.voucherDiscount)),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(
              S.current.total, getDisplayMoney(_con.order!.totalBeforeTax),
              isBold: true),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(S.current.VAT, getDisplayMoney(_con.order!.tax)),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(S.current.grandTotal.toUpperCase(),
  ...
  Container _createDeliverToContainer(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          OrderSummaryRow(
              S.current.fullName, _con.order!.deliveryAddress!.fullName!,
              txtAlign2: TextAlign.start),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(S.current.phone, _con.order!.deliveryAddress!.phone,
              txtAlign2: TextAlign.start),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(S.current.date, _con.order!.getDeliverDateSlot,
              txtAlign2: TextAlign.start),
          Divider(thickness: 1, color: Colors.grey.shade400, height: 5),
          OrderSummaryRow(
              S.current.address, _con.order!.deliveryAddress!.getFullAddress,
              txtAlign2: TextAlign.start),
        ],
      ),
    );
...
//                  Product product = c.product;
            ...
            SizedBox(height: 20),
            TextButton(
                child: Text(S.current.cancelOrder),
...
class OrderSummaryRow extends StatelessWidget {
  ...
  OrderSummaryRow(this.text1, this.text2,
      {this.isBold = false,
      this.txtAlign1 = TextAlign.start,
      this.txtAlign2 = TextAlign.end});
