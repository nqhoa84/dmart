...
class PromotionScreen extends StatefulWidget {
  ...
  @override
  _PromotionScreenState createState() =>
      _PromotionScreenState(promo: this.promotion!, promotionId: promotionId!);
}

class _PromotionScreenState extends ProductStateMVC<PromotionScreen> {
  Promotion? promo;
  int? promotionId;
  _PromotionScreenState({this.promo, this.promotionId})
...
//  }
  ...
  Future<void> loadMore() async {
    int pre =
        proCon.promotionProducts != null ? proCon.promotionProducts!.length : 0;
    proCon.listenForPromoProducts(promo!.id, nextPage: true);
    canLoadMore = proCon.promotionProducts!.length > pre;
  }
